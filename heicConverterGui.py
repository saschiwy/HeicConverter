...
class HEICConverterGUI:
    ...
    def convert(self):
        ...
        if os.path.isdir(path):
            ...
            converted = convert_heic_to_jpeg(path, recursive, overwrite, remove, self.quality_scale.get())
            output_text += f'Successfully converted {len(converted)} files\n'

        elif os.path.isfile(path):
            output_text += f'Converting HEIC file {path}\n'
            convert_heic_file(path, os.path.splitext(path)[0] + ".jpg", overwrite, remove, self.quality_scale.get())
